...
> **⛔ Last resort — API first.** Driving the UI is only for (a) the specific form/page/portal/workspace that is itself under test or inspection, or (b) an operation with genuinely no API path (say which). Everything else — prerequisite/test records, setup/teardown, reads, backend verification — goes through `servicenow_api` (or `servicenow_run_script`, admin-only). Never create prerequisite records through a form that isn't under test. Impersonate with `iframe_tool` action `impersonate` (REST-based; `user: "stop"` to end) — never via the UI impersonation menu.
...
- **API first.** If the job maps to a UI Action or record producer, prefer the Table API / `servicenow_run_script` (admin-only — check `list_instances` roles first). Drive the UI only when there is genuinely no API path, or the task *is* "operate the rendered form". Seed prerequisite data and verify results via `servicenow_api`, not the UI.

...
- **⚠️ Custom (non-catalog) SP widgets do NOT expose `#s2id_sp_formfield_*` at all.** That id is specific to the **standard catalog `sp_form` widget**. On a custom widget (verified live) the select2 keeps a **persistent autogen id** (`#s2id_autogen11`, …) that is **never** replaced — here the autogen id is the **real, drivable node**, *not* a transient trap. Anchor instead to the widget's **stable custom wrapper class** (e.g. `.select2-container.<widget-specific-class>`), or label-derive the live container id at runtime. **Which world am I in?** If `#s2id_sp_formfield_<var>` exists → catalog form (autogen = transient, wait for the stable id). If it doesn't but a select2 with a stable custom class does → custom widget (autogen = permanent, anchor the custom class). Either way the **open/commit mechanic below is identical** — only the anchor changes; always verify by confirming the drop opens (`get_properties("#select2-drop").properties.visible===true`).

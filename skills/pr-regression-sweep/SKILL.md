...
- **Sub-agents do the heavy reading.** Patch dumps and greps stay out of the parent context.
...
4. **Spawn reviewers** (`spawn_sub_agent`, pool max = 2 concurrent — serialize beyond that):
   ...
   - Use `wake_parent: false` and collect with `await_all` on the spawn handles.

5. **Triage & fix**:
   - Deduplicate findings; re-verify each critical/major finding yourself with one targeted `workspace` `read` before editing.
   - Fix critical + major (and trivial minors) with `workspace` `edit`. Skip speculative refactors.

...
2. `servicenow_api` GET on `syslog` with `query: level=2^sys_created_on>javascript:gs.hoursAgoStart(24)`, fields `level,source,message,sys_created_on`, limit 200
...
2. `servicenow_api` GET on `incident` with `query: priority=1^active=true`, fields `number,short_description,assignment_group,assigned_to,opened_at,sys_updated_on,state`, `url_params: { sysparm_display_value: 'true' }`

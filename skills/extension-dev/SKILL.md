...
5. **Push (immediately — do NOT wait for user confirmation)**: Pushing the PR is part of finishing the task, in the SAME turn as the final edit. As soon as your edits are complete and self-verified (déroulement, diff re-read, or automated checks — user confirmation is NOT a prerequisite), run `workspace push` yourself. Never end a task with "once you confirm it works I'll push the PR" or any variant that gates the push on the user testing/reloading first — the PR itself is the review artifact: the user reviews, requests changes, or merges on GitHub, and follow-up commits can be appended to the same PR (same `branch_name`) if testing reveals problems. **Appending to a PR never switches branches**: stay on the workspace you are on (e.g. `::main`) and run `workspace push { branch_name: "fix/x", files: [...], commit_message: "..." }` again with the same `branch_name` — `pr_reused: true` confirms the append. Do NOT `clone` the PR branch, do NOT use the `branch` action, do NOT pass `workspace: owner/repo::<pr-branch>`. Only skip the push if the user explicitly said not to push, or the work is knowingly exploratory/incomplete. **By default, pass `files: [...]` listing ONLY the files YOU worked on in this chat** — other dirty files in the workspace may be work-in-progress from other agents/chats, and pushing them would leak unrelated, possibly half-finished changes into your PR. Run `workspace status` first: it shows which chat owns each dirty file. Only omit `files` when you have verified every dirty file belongs to your task.
...
- **`branch` action (local fork)**: `workspace branch` with `branch: "feat/x"` forks the current workspace into `owner/repo::feat/x` locally — a cheap row copy; the **remote branch is only created on the first push** from the fork (cut from the fork's base branch; `branch_name` defaults to the fork's own branch). Dirty edits travel to the fork by default (`move_dirty: true` reverts the source clean); pass `move_dirty: false` to keep them in both. The fork is pinned automatically, so Reload builds it. **NOT for appending to an existing PR** — `branch` is only for deliberately starting a separate local line of work; it moves the pin and strips dirty files from the source. Never fork to an existing PR head branch; to add a commit to your PR, `push` again from the current workspace with the same `branch_name`. Dirty files hard-locked by ANOTHER chat (cross-chat conflict) do not travel: the fork gets their pristine base version, their edits stay in the source workspace, and the response lists them in `left_behind`.
- **On a `cross_chat_conflict`**: FIRST, if the conflicting file is one you already pushed to YOUR PR, or the other chat is dormant/closed, use `force: true` on the mutation (and `files: [...]` on push) and keep pushing to the same `branch_name` from the current workspace. Forking with `branch` is a LAST resort and must use a NEW branch name that is not an existing PR head.
...
Use top-level `run_tests` (never nest it in js_eval). It captures `test/harness.js` and selected `test/*.test.js` from the workspace, then evaluates them sequentially in a host-guarded sandbox. No Node, require, fs or process; top-level await is supported.

### Write behavioral tests against real source

- Register synchronous `describe(name, fn)` blocks and async `test(name, fn, {tags:['unit'], timeout:2000})` bodies; use `beforeEach/afterEach`, `test.skip` or `skipTest(reason)` when appropriate. Assert observable results/calls, not implementation substrings. Every passing test needs `assert.*` (zero assertions fail unless explicitly `noAssert:true`). Use `strictEqual/deepStrictEqual/match/throws`; **await** `assert.rejects`.
- `loadFile(path)` returns raw workspace text. `runFile(path,args)` evaluates it locally via `evalModule`: top-level return wins, then CommonJS exports, then a namespace of column-zero declarations. Exported functions remain callable in the same sandbox.
- `loadModules([paths], {globals:{...}})` reads and evaluates real browser-global files in one ordered scope. Load actual dependencies first; stub only external effects (Chrome, DOM, storage, tool dispatch). Unknown globals throw; bare `typeof X` probes are lenient and recorded in `m.__unstubbed`. Avoid blanket `lenient:true`. `fakeChrome()/fakeWindow()` are minimal in-memory helpers, not a live browser. This loader does not support top-level return.

Runnable `test/*.test.js` example (the validator really depends on the policy):

```js
test('validator accepts only constrained read grants', async function() {
    var m = await loadModules([
        'src/js/core/075-test-run-policy.js',
        'src/js/tools/160-run-tests.js'
    ]);
    assert.strictEqual(m.rtValidateArgs({
        allow_tools: [{ name: 'workspace', actions: ['status'] }]
    }).ok, true);
    assert.strictEqual(m.rtValidateArgs({
...
### Local evaluation vs real tool calls

The sandbox's `executeTool(name,args)` posts `sandboxToolCall` to its parent; replies resolve/reject pending promises by ID. `loadFile` uses this bridge only for `run_js_file` **mode:source**; `runFile/loadModules` then evaluate locally. Calling the **tool** `run_js_file` in module/script mode would instead request nested execution and is forbidden during tests. Inject an in-memory `executeTool` into production modules to test effects without invoking live tools. For loader/bridge changes, follow `test/sandbox-loader-bridge.test.js`: evaluate the actual workspace sandbox script with fake peers, not copied functions or just the installed loader.

**Host restrictions:** all real mutations (including scratch writes/cleanup), cookies, foreign workspaces, force, nested js_eval/run_tests and live network tools are denied. Default safe calls are source loading, workspace **read/ls/grep** (not list), and sleep; status/diff require top-level `allow_tools:[{name:'workspace',actions:['status','diff']}]`. Test policy-denial cases with isolated fake dispatch, not deliberately denied real calls: host denials fail the whole run even if caught. This is a tool boundary, **not hostile-JavaScript isolation**; direct JS networking/CSP are unchanged.

### Verify and report honestly

1. Run focused files first: `run_tests {files:['test/real-modules.test.js','test/sandbox-loader-bridge.test.js'], tags:['unit']}`, then the full default `run_tests {}` (120s overall / 30s per test defaults; bounded overrides supported). Any timeout revokes authority, destroys the exact frame and aborts remaining work; cleanup hooks do not run after timeout.
2. Default tags are unit+contract+canary. **Contract and runtime layers are currently unsupported skips**, including in dev mode; scratch helpers do not grant write permission. Fake-peer tests are unit coverage, not live Chrome/ServiceNow contracts. Legacy `await registerRunner(name, fn)` adapts old runners, but executes them during registration before per-test filtering/timeouts: prefer native tests.
3. Before push, include exact files/total/passed/failed/skipped, per-file results, isolation and denied calls in the PR. Check nonzero execution: no matched files can return success without testing anything. Reproduce unrelated failures separately and disclose them; never call a red full suite green. Source/test edits load from the workspace immediately; changes to the installed runner/sandbox infrastructure still need the user's Reload for end-to-end verification.
...
- **`workspace push` reuses a PR when you push to an existing branch** — a new `branch_name` creates a new branch + new PR against the base/source branch we cloned from; passing the SAME `branch_name` as a prior push appends another commit to that branch and updates the existing open PR (each commit carries the full current changes against the base). To add commits to the same PR, push again with the same `branch_name` (the response sets `pr_reused: true` when an existing PR was updated). **Appending to a PR never switches branches** — run it from `::main` (the workspace you are already on) again, e.g. `workspace push { branch_name: "fix/x", files: [...], commit_message: "..." }`; do NOT `clone` the PR branch, use the `branch` action, or pass `workspace: owner/repo::<pr-branch>`.

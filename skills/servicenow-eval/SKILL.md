...
**v5.1 (2026-07-16) — doc update from post-run learnings** (flaky-instance run): clarified setup-retry semantics under client network errors, added stale-setup-lock guidance, and documented the durable-integrity response fields shipped in **PR #700** (branch `fix/eval-runner-durable-integrity`, `src/js/core/085-eval-runner.js`). Those new response fields are NOT active until PR #700 is merged AND the extension is rebuilt + Reloaded — the currently-built runner still exhibits the OLD behavior described alongside each field.

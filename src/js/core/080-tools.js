...
var TOOLS = [
                ...
                properties: {
                    ...
                    edits: {
                        ...
                        items: {
                            type: 'object',
                            properties: {
                                find: { type: 'string', description: 'Unique text to find (unless replace_all or occurrence is set). Include enough context (few lines) to ensure uniqueness.' },
        ...
        function: {
            ...
            parameters: {
                ...
                properties: {
                    ...
                    expected_version: { type: 'integer', description: 'Required for edit_html. Read the saved widget via html_widget action=read first.' },
                    operation_id: { type: 'string', description: 'Stable retry key for edit_html. Reuse with identical edits and expected_version only.' },
    ...
    },
    {
        ...
        function: {
            name: 'widget_eval',
            description: 'Run an async JavaScript body in one exact LIVE widget render, never a saved widget or offscreen clone. action:list discovers connected renders (IDs expire on rerender/close); action:eval requires instance_id and code. Returns {success,instance_id,result} or {success:false,error,code}. Execution is supported only in identified top-level chats (including start_chat background chats); sub-agents may list but cannot execute, even with a full roster. Existing widget tool permissions apply to callbacks; no per-callback agent provenance or cancellation guarantee. JSON-compatible results only, 65536 characters maximum; undefined becomes null. Timeout bounds waiting, NOT execution: synchronous infinite loops can freeze the iframe and async side effects may continue. No implicit target or saved-id fallback.',
            parameters: {
                type: 'object',
                properties: {
                    action: { type: 'string', enum: ['list', 'eval'] },
                    instance_id: { type: 'string', description: 'Exact live render ID from list or start_chat origin context; required for eval.' },
                    code: { type: 'string', description: 'Async function body; use return for a JSON-compatible result. Runs in the widget sandbox, not the extension panel.' },
                    ...
                    confirm: { type: 'boolean', description: 'Request user review before script execution.' },
                    status_message: { type: 'string', description: 'Short human-friendly status.' }
                },
                required: ['action']
            }
        }
    ...
    },
    {
        ...
        function: {
            name: 'run_js_file',
            description: 'Load or run a JavaScript file from the cloned workspace INSIDE the js_eval sandbox. mode "module" (default): evaluates the file CommonJS-style and returns its exported value (a top-level `return X`, else module.exports, else a namespace object of the file\'s top-level function/var declarations — works for browser-global-style src files); `args` is available to the file. mode "script": the file body IS the js_eval code (may `return`, call executeTool/sleep). mode "source": returns the raw file text without executing. Inside js_eval the same bridge is exposed as `await loadFile(path)` (source string) and `await runFile(path, args)` (exported value, evaluated in the calling sandbox so returned functions stay callable). Results of module/script mode must be JSON-serializable. CAVEAT mode "script": the file body travels as js_eval `code`, so the js_eval Unicode-quote sanitizer rewrites curly quotes \u2018\u2019\u201c\u201d and backtick lookalikes to ASCII inside string literals too — use mode "module" (or loadFile) for files whose string contents must survive byte-for-byte.',
            parameters: {
                type: 'object',
                properties: {
    ...
    },
    {
        ...
        function: {
            name: 'run_tests',
            description: 'Run test/*.test.js in a HOST-GUARDED read-only js_eval invocation (top-level caller only). Use loadFile/runFile for source in the pinned workspace. All real mutations INCLUDING scratch writes, cookies, nested js_eval/run_tests and run_js_file module/script are forbidden. Contract and runtime layers are temporarily unsupported and explicitly skipped, never claimed as live contract passes. Host snapshots fail closed on missing/failed/capped reads; host denials cannot be erased by sandbox results. Every timeout revokes authority and destroys the exact frame. Returns success, summary, per-file results, host isolation and denied_calls. Default tags: unit, contract, canary. allow_tools accepts only constrained safe action grants, not legacy tool-name strings. This is a tool boundary, NOT hostile-JavaScript isolation: direct JS network/fetch and global sandbox CSP are unchanged. Call after editing; include exact verification and unsupported coverage in the PR.',
            parameters: {
                type: 'object',
                properties: {
                    files: { type: 'array', items: { type: 'string' }, description: 'Workspace paths of test files to run. Default: every test/*.test.js.' },
                    pattern: { type: 'string', description: 'Regex applied to the file paths to narrow the selection, e.g. "harness|canary".' },
                    ...
                    allow_tools: { type: 'array', description: 'Top-level opt-in safe actions only. Legacy strings and mutation grants are rejected.', items: { type: 'object', properties: { name: { type: 'string', enum: ['workspace'] }, actions: { type: 'array', items: { type: 'string', enum: ['status', 'diff'] } } }, required: ['name', 'actions'], additionalProperties: false } },
                    workspace: { type: 'string', description: 'Workspace key (owner/repo::branch). Default: the current/pinned workspace.' },
                    status_message: { type: 'string', description: 'Human-friendly status message describing what this tool call is doing (shown in UI header)' }
                },
                required: []
        ...
        function: {
            ...
            parameters: {
                ...
                properties: {
                    ...
                    workspace: { type: 'string', description: 'Workspace identifier (owner/repo::branch). Optional — defaults to the current workspace if omitted. Do NOT point this at a PR head branch (owner/repo::<pr-branch>) to append to that PR — push from the workspace you are on with the same branch_name instead.' },
                    branch: { type: 'string', description: 'Branch name (for clone: branch to clone; for branch action: the NEW branch name to fork to — never an existing PR head, forking is NOT how you append to a PR; for push: ignored — use branch_name to control the push target)' },
                    ...
                    branch_name: { type: 'string', description: 'Branch name for push. A new branch+PR are created if it does not exist; reuse the SAME branch_name from a prior push to append another commit to that existing PR. To append, run push again FROM THE SAME WORKSPACE you are on (e.g. owner/repo::main) — the workspace never changes branch. Do NOT clone the PR branch, do NOT use the branch action, do NOT pass workspace: owner/repo::<pr-branch>. pr_reused: true in the response confirms the append.' },
                    ...
                    include_prs: { type: 'string', enum: ['lean', 'full'], description: "For status: 'lean' (default) returns {number,title,state,url,branch,merged_at?} per PR with merged PRs capped to the 3 most recent; 'full' returns every stored PR entry including its files[] (large)." },
                    ignore_case: { type: 'boolean', description: 'For grep: case-insensitive matching (default true — a leading (?i) in the pattern is also accepted and stripped). Pass false for a case-sensitive grep.' },
    ...
    // collect them.
    {
        ...
        function: {
            name: 'await_handle',
            description: 'Block (on the scheduler, not the model) until an async handle resolves. Handles are issued by sub-agent operations — spawn_sub_agent, wake_sub_agent, and agent_message return {handle} receipts that settle when the sub reports. Returns the snapshot {status: done|error|cancelled|pending, result?, error?}. For a sub-agent handle: status "done" → snapshot.result is the sub\'s full report {status (done|need_input), summary, data, artifacts, from, from_name, at}; status "error" → snapshot.error is the headline (the report summary) AND snapshot.result is the full report object; status "cancelled" → snapshot.error is the stop reason (no result). If status is still "pending" after timeout_ms, the handle is left in-flight — you can await it again. Handles are per-chat and do not survive a page reload.',
            parameters: {
                type: 'object',
                properties: {
        ...
        function: {
            ...
            parameters: {
                ...
                properties: {
                    ...
                    summary_cap_kb: { type: 'integer', description: 'Max size in KB of the sub\'s report_to_parent summary (default 4, clamped to 1–16). The registry truncates longer summaries with an explicit "[truncated by registry]" marker. Raise it only when the sub must return a long structured result inline; prefer artifacts (docs/files) for big payloads.' },
                    status_message: { type: 'string', description: 'Human-friendly status message describing what this tool call is doing (shown in UI header)' }
                },
...
var TOOL_SHORT_DESCRIPTIONS = {
    ...
    run_tests: 'Run test/*.test.js inside the js_eval sandbox with test/harness.js (tags: unit/contract/runtime/canary); call before pushing.',

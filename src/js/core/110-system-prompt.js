...
var DEFAULT_SYSTEM_PROMPT_TEMPLATE = [
    ...
    '  • `await_all({ handles, timeout_ms? })` — wait for every handle. Returns `{ snapshots: [...], timedOut }` in the same order as input. `timedOut` is true when at least one handle never settled within `timeout_ms` (mirrors `await_any`\'s `timeout`).',
    ...
    '',
    'PROACTIVELY DELEGATE AS CONTEXT GROWS: model quality degrades at long context, so do not wait until you are forced to. Once a conversation is getting large (roughly past 70k tokens), prefer spawning a sub-agent for ANY heavy or verbose step — reading/searching many files, scanning logs, multi-record investigation, iterative trial-and-error — instead of doing it inline. Keep the main thread lean so your reasoning stays sharp. The runtime will also remind you when context crosses ~70k.',
    '',
    'Usage: `spawn_sub_agent({instructions:"...", name:"...", context_seed:{...}})` returns `{agent_id, chat_id, handle}`. Then `await_handle(handle)` to collect when you need the result. Several spawns can run in parallel — pool size is 2 concurrent, excess queues. The pool default is intentionally low because Anthropic enforces an account-level concurrent-request cap above us; even 2 parallel subs can occasionally bump 429 on fresh tiers. If you need a longer fan-out, serialize. Pass `output_schema:{...}` to declare the exact shape the sub must return in `report_to_parent`\'s `data` field — it is injected into the sub\'s first message with a conform-exactly directive, so when you spawn + await + parse inside one js_eval you can safely destructure `snapshot.result.data`.',
    '',

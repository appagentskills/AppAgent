...
// SECTION_ICON_SHORTCODES (shortcode → emoji map) and replaceEmojiShortcodes
// moved to src/js/core/055-emoji-shortcodes.js (core tier) so EVERY markdown
// render path — not just the sub-agent surfaces — can reach them.
// _SECTION_EMOJI_RE / _liftSectionIcon / _applySectionIcons below still read
// the global SECTION_ICON_SHORTCODES map from there.
...
function renderSubReport(msg, index) {
    ...
    // cap shift()ed out of msg.phases were never overwritten or pruned, so
    // both stores grew (FIX5: gcRawCopyStore now ALSO sweeps DOM-orphaned
    // 'sub:' keys, but this prune stays — it is what cleans _subPanelPref,
    // and it drops stale-phase copy keys even while the card is in the DOM)
...
function updateSidebarWorkerMetrics() {
    ...
    for (var i = 0; i < cards.length; i++) {
        ...
        // Tool-call counter (display only — no cap) — authoritative live value
...
function updateSubAgentSelfCard() {
    ...
    if (parentChatId && chats[parentChatId]) {
        ...
        var backIcon = (typeof UI_ICONS !== 'undefined' && (UI_ICONS.backToParent || UI_ICONS.back)) ? (UI_ICONS.backToParent || UI_ICONS.back) : '\u21B0';
...
function _reconstructSubsFromMessages(chatId) {
    ...
    for (var i = c.messages.length - 1; i >= 0; i--) {
        ...
        out.push({
            ...
            // Display-only counter; reconstructed cards never carry a cap
            // field (there is no tool-call limit).
